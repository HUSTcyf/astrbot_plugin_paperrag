...
#### 10. 移除正则参考文献解析，仅保留 LLM 方案

**文件**: `reference_processor.py`, `hybrid_parser.py`
...
**新增**: 模块级 `_find_reference_section()` 函数，使用关键词匹配定位参考文献部分。

**hybrid_parser.py 引用更新**: 移除已删除函数的 import（`ReferenceExtractor`、`process_references_and_citations` 等），引用处理逻辑移除正则 fallback，仅使用 LLM 解析。

**效果**: `reference_processor.py` 从 1913 行缩减至 ~890 行（减少 54%）。
...
**修复**: `_call_llm()` 中 `max_tokens` 从 8192 提升至 16384。

---

#### 12. 修复 CRAG 评估 JSON 解析

**文件**: `hybrid_rag.py`

**问题**: LLM 返回的 JSON 中 reasoning 字段包含未转义引号，导致 `json.loads()` 失败。

**修复**: 使用正则 fallback 直接提取 JSON 字段值（score/level/reasoning），不依赖 `json.loads()`。同时 `max_tokens` 从 200 提升至 1024，避免输出被截断。

---

#### 13. 修复 LlamaCppVLMProvider 忽略调用者传入的 max_tokens

**文件**: `llama_cpp_vlm_provider.py`

**问题**: `text_chat()` 方法接受 `max_tokens` 参数但从未使用，始终使用 `self.max_tokens`。

**修复**: `text_chat()` 改为使用 `kwargs.get('max_tokens', self.max_tokens)` 接受调用者传入值。
